"""Matched five-round trial, conservative adoption, then automatic curriculum resume."""
import copy
import json
import os
import random
...
DATA = Path('G:/ctfn-text/data/v3_1_balanced_validation_v4')
...
def run():
    ...
    os.write(fd,canonical({'pid':os.getpid(),'output':str(ROOT/'subskill_probe')}).encode());os.close(fd)
    production=ROOT/'math'; start=production/'current.specialist'
    root=ROOT/'subskill_probe'; root.mkdir(exist_ok=False)
    ...
    try:
        ...
        assert idx==1 and meta['cursor']==0 and meta['dataset_hash']==file_hash(DATA/'manifest.json')
        ...
        for arm in ('start','baseline','targeted'):
            ...
            status(state='starting',phase=manifest['stages'][idx]['name'],stage_index=idx,
                   ...
                   reason=f'Subskill recovery comparison: {arm}. Production checkpoint protected.',
            ...
            if arm!='start':
                for offset in range(ROUNDS):
                    ...
                    rows=(sample(expanded,prior,focus,policy['examples'],seed) if arm=='targeted'
                    ...
                    status(state='training',epoch=offset+1,steps=len(chunks),step=0,
                           focused_criterion=focus if arm=='targeted' else None,evaluation=None)
                    losses=[];model.train()
        ...
        adopted_meta.update(consecutive=0,cursor=0)
