...
root=Path('G:/ctfn-text/artifacts/v3_1/math');data=Path('G:/ctfn-text/data/v3_1_equality_v2')
assert json.loads((root/'status.json').read_text())['state']=='paused'
assert (root/'equality_diagnostic_before.json').exists()
...
backup=root/f"before_equality_repair_round_{m['round']}.specialist";assert not backup.exists();shutil.copy2(p,backup)
m['dataset_hash']=file_hash(data/'manifest.json');m['cursor']=0;m['consecutive']=0
m['controller'].update(mode='repair',focus='1NPV-2',recovery_queue=['1NPV-2'],repair_done=0,recovery_gate_streak=0,focus_streak=0,streak=0,full_streak=0,full_pass_round=None,validation_enabled=True,validation_start_round=m['round'],normal_done=0,recovery_blocks=0,attempt_counts={})
...
(root/'equality_adoption.json').write_text(json.dumps(report));print(json.dumps(report))
